        ...
        RULES: {
          ...
          soDienThoai: () => ({
            validator(_, value) {
              ...
              if (value && !pattern.test(value) === true) {
                return Promise.reject(new Error(`Không đúng định dạng của số điện thoại`));
              }
          ...
          chiNhapSoThapPhan: () => ({
            validator(_, value) {
              if (value && !/^\d+(\.\d+)?$/.test(value)) {
                return Promise.reject(new Error("Chỉ nhập số hoặc số thập phân"));
              }
              return Promise.resolve();
            },
          }),
            ...
            return ({ getFieldValue }) => ({
              validator(_, value) {
                ...
                if (value && number && value > number) {
                  return Promise.reject(new Error(`Phải nhỏ hơn hoặc bằng ${name} ${digit}`));
                }
                return Promise.resolve();
              },
            });
              ...
              validator(_, value) {
                ...
                if (
                  ...
                  if (!(value + "")?.includes("00010101") && value + "" != "0") {
                    return Promise.reject(new Error(`Phải lớn hơn hoặc bằng ${name} ${digit}`));
                  }
                }
                return Promise.resolve();
              },
            ...
            return ({ getFieldValue }) => ({
              validator(_, value) {
                ...
                if (
                  ...
                  if (!(value + "")?.includes("00010101") && value + "" != "0") {
                    return Promise.reject(new Error(`Phải lớn hơn ${name} ${digit}`));
                  }
                }
                return Promise.resolve();
              },
              ...
              validator(_, value) {
                ...
                if (value != undefined && value != null) {
                  if (
                    ...
                    if (!(value[1] + "")?.includes("00010101") && value[1] + "" != "0") {
                      return Promise.reject(new Error(`Phải nhỏ hơn ${name}`));
                    }
                  }
                }
          ...
          percentAndRatio: () => ({
            validator(_, value) {
              ...
              if (!percentPattern.test(value) && !ratiotyLePattern.test(value)) {
                return Promise.reject(
                  new Error("chỉ có thể nhập dạng: phần trăm 'số + %' hoặc tỷ lệ 'số:số' !")
                );
              }
          ...
          percent: () => ({
            validator(_, value) {
              ...
              if (!percentPattern.test(value) && !percentPattern2.test(value)) {
                return Promise.reject(new Error("chỉ có thể nhập dạng: phần trăm 'số + %'!"));
              }
            ...
            validator(_, value) {
              if (value) {
                ...
                if (!ratiotyLePattern.test(value)) {
                  return Promise.reject(new Error("Chỉ có thể nhập dạng tỷ lệ 'số:số' !"));
                }
              }
              return Promise.resolve();
            },
            ...
            validator(_, value) {
              if (value) {
                ...
                if (!pattern.test(value)) {
                  return Promise.reject(
                    new Error("Chỉ có thể nhập số cách nhau bởi dấu | (vd: x1|x2|x3)")
                  );
                }
              }
          ...
          validatePassword: () => ({
            validator(_, value) {
              ...
              if (!lengthRegex.test(value)) {
                return Promise.reject(new Error("Mật khẩu phải từ 8 đến 20 kí tự"));
              }
            ...
            DATETIME_DATABASE: {
              getValueProps: (i) => ({
                value:
                  i &&
                  i != 0 &&
                  (typeof i !== "string" || (!i.includes("0001-01-01T00:00:00") && i != "0"))
...
export const formatPrice = (value) => `${value}đ`.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
