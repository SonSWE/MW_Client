...
import { columnWorkingHistory } from "./comom";
import { FormFreelancer } from "../../../const/FormFreelancer";

const WorkingHistoryTab = ({ formInstance, action, disabled }) => {
  ...
  const handleRemoveRows = (type) => {
    let datas = convertToArray(formInstance.getFieldValue(FormFreelancer.WorkingHistories));
    let _rowSelected = gridRef?.current?.api.getSelectedRows();

    if (_rowSelected?.length > 0) {
      formInstance.setFieldValue(FormFreelancer.WorkingHistories, [
  ...
  const handleAddRowAsync = async () => {
    const value = await formInstance.getFieldValue(FormFreelancer.WorkingHistories);
    ...
    if (data.length === 0) {
      await formInstance.setFieldValue(FormFreelancer.WorkingHistories, [newRow]);
    } else {
      await formInstance.setFieldValue(FormFreelancer.WorkingHistories, [...data, newRow]);
    }
  ...
  const onCellEditingStopped = (params) => {
    ...
    const { data } = params;
    const oldDatas = formInstance.getFieldValue(FormFreelancer.WorkingHistories);

    if (!validateEdit(oldDatas, data, params.rowIndex)) {
      formInstance.setFieldValue(FormFreelancer.WorkingHistories, [...oldDatas]);
    } else {
      ...
      formInstance.setFieldValue(FormFreelancer.WorkingHistories, [...rowData]);
    }
  };

  const _rowProps = (type) => {
    let list = convertToArray(formInstance.getFieldValue(FormFreelancer.WorkingHistories));
  ...
  return (
    <div className="row-item">
      <GroupBox title={"Danh sách lịch sử làm việc"} className="mt-4">
        ...
        <Form.Item
          ...
          shouldUpdate={(prevValues, currentValues) =>
            isRender(prevValues, currentValues, [FormFreelancer.WorkingHistories])
          }
        >
...
export default WorkingHistoryTab;
