...
import { columnProposal } from "./comom";

const ProposalTab = ({ formInstance, action, disabled }) => {
  ...
  const handleRemoveRows = (type) => {
    let datas = convertToArray(formInstance.getFieldValue(FormJob.Proposals));
    let _rowSelected = gridRef?.current?.api.getSelectedRows();

    if (_rowSelected?.length > 0) {
      formInstance.setFieldValue(FormJob.Proposals, [
  ...
  const handleAddRowAsync = async () => {
    const value = await formInstance.getFieldValue(FormJob.Proposals);
    ...
    if (data.length === 0) {
      await formInstance.setFieldValue(FormJob.Proposals, [newRow]);
    } else {
      await formInstance.setFieldValue(FormJob.Proposals, [...data, newRow]);
    }
  ...
  const onCellEditingStopped = (params) => {
    ...
    const { data } = params;
    const oldDatas = formInstance.getFieldValue(FormJob.Proposals);

    if (!validateEdit(oldDatas, data, params.rowIndex)) {
      formInstance.setFieldValue(FormJob.Proposals, [...oldDatas]);
    } else {
      ...
      formInstance.setFieldValue(FormJob.Proposals, [...rowData]);
    }
  };

  const _rowProps = (type) => {
    let list = convertToArray(formInstance.getFieldValue(FormJob.Proposals));
  ...
  return (
    <div className="row-item">
      <GroupBox title={"Danh sách đề xuất công việc"} className="mt-4">
        ...
        <Form.Item
          ...
          shouldUpdate={(prevValues, currentValues) =>
            isRender(prevValues, currentValues, [FormJob.Proposals])
          }
        >
